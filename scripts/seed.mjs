...
const text = (markdown) => ({ id: blockId(), type: 'text', markdown });
const heading = (headingText, level = 2) => ({ id: blockId(), type: 'heading', text: headingText, level });
const divider = (style = 'rule') => ({ id: blockId(), type: 'divider', style });
const image = (url, alt, opts = {}) => ({ id: blockId(), type: 'image', url, alt, ...opts });
const ps = (psText, recipeId = null) => ({ id: blockId(), type: 'ps', text: psText, recipeId });
...
INSERT INTO posts (slug, title, dek, blocks, status, published_at, created_at, updated_at)
VALUES (@slug, @title, @dek, @blocks, 'published', @published_at, ${now}, ${now})
`);
...
insertPost.run({
  ...
  blocks: JSON.stringify([
    text(
      `Nothing happened this week. I keep waiting to feel bad about that, and the feeling hasn't turned up yet, so I'm writing it down before it does.
...
I think the trouble is that we've been taught to file weeks like receipts — proof of something, in case anyone asks. Nobody asks. And when they do, "it was quiet" is a complete answer, delivered by someone who slept.`,
    ...
    ps('I braised leeks twice this week, which is the closest thing to news.', leeks.lastInsertRowid),
  ]),
...
insertPost.run({
  ...
  blocks: JSON.stringify([
    text(
      `The box was in the loft, under a bag of curtain rings nobody has ever needed.
...
Til done. As if the cake and I have an understanding.`,
    ...
    heading('a box of private notes', 2),
    text(
...
insertPost.run({
  ...
  blocks: JSON.stringify([
    text(
      `At six the streets are wet even when it hasn't rained, and there are exactly three kinds of people out: bakers, dog owners, and men in hi-vis standing near a hole with real authority.
...
insertPost.run({
  ...
  blocks: JSON.stringify([
    text(
      `I did all of them in one afternoon, which was a mistake, and I would do it that way again.
...
Nine of the eleven were root-bound. Not dying — worse than dying, holding steady. Each one had made a dense white knot of itself in the shape of a container I had chosen for it in a shop, years ago, for no reason at all.`,
    ...
    image('/placeholder.svg', 'A row of repotted plants on the kitchen windowsill.', {
      caption: 'the whole afternoon, in one windowsill',
      width: 'wide',
    }),
    text(
